...
export default function httpApi<S extends { new(session: object, captures?: {[name: string]: string}): I }, I extends {[K in keyof I]: Function}>(Ctor: S): Handler {
    return async (req, res, captures) => {
        ...
        if (typeof func !== 'function') throw new Error(`rpc: invalid API has no function '${funcName}'`); // TODO: revise error handling
