...
/**
 * A rule set provides a deterministic means to generate an appropriate response for a given address
 * and request, according to the list of rules passed to the constructor. The rule list passed to
 * the constructor is unordered, with each rule consisting of a pattern and a handler. For each
 * rule, the pattern specifies the set of addresses that it accepts, and optionally captures some
 * substrings from the address. The corresponding handler generates a response from a request,
 * using the substrings captured by the pattern, if any. Handlers may also be 'decorators'. A
 * decorator is a handler function with a `$next` formal parameter. These are treated specially in
 * the response-generation process, as described below.
 *
 ...
 *     handler signifies that the handler declined to handle the request. In this case, the next
 *     handler is tried. The handlers are thus tried in order until one returns a non-null value,
 *     which is the response. Subsequent handlers are not tried.
 * (5) The previous step (4) differs when some of the handlers are decorators. A decorator is always
 *     executed *before* its more specific handlers. The more-specific handlers are aggregated into
 *     a collective 'downstream' handler, and passed to the decorator via its `$next` parameter. A
 *     decorator may thus control the downstream handling process in many ways. It may execute logic
 *     before and/or after executing the downstream handlers. It may decline to execute the
 *     downstream handlers altogether, thus acting as a filter. It may send a modified request to
 *     the downstream handlers, and/or it may modify the response of the downstream handlers before
 *     returning it.
 *
