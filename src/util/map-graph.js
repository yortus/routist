...
function mapGraph(graph, addNode, addEdge) {
    var newRoot = new Map().set(null, graph);
    return mapGraphNode(newRoot, null, addNode, addEdge, new Map());
}
 ...
 */
function mapGraphNode(graph, inputNode, addNode, addEdge, visited) {
    // If we've already visited this input node, return its already-constructed output node.
    var cycle = visited.get(inputNode);
    if (cycle) {
        if (graph.get(inputNode) !== cycle[0]) {
            var _0 = graph.get(inputNode);
            var _1 = cycle[0];
            debugger;
        }
    ...
    var outputNode = addNode(inputNode);
    var children = graph.get(inputNode);
    visited.set(inputNode, [children, outputNode]);
    // Recurse over the input node's child nodes.
    Array.from(children.entries()).forEach(function (_a) {
        var childNode = _a[0], grandChildren = _a[1];
        ...
        var childOutputNode = mapGraphNode(children, childNode, addNode, addEdge, visited);
