...
var get_function_parameter_names_1 = require('../../src/utils/get-function-parameter-names');
describe("Identifying a function's formal parameters", function () {
    ...
    var has = { REST_PARAMS: true, DEFAULT_PARAMS: true, DESTUCTURING: true, ASYNC: true, ASYNC_ARROW: true };
    try {
        eval("(function (...rest) {})");
    }
    catch (ex) {
    ...
    tests = tests
        .map(function (test) { return test.split('] ').map(function (s) { return s.replace(/^\[/, ''); }); })
        .filter(function (parts) { return parts.length === 1 || has[parts[0]]; })
        .map(function (parts) { return parts[parts.length - 1]; });
    tests.forEach(function (test) {
        it(JSON.stringify(test).slice(1, -1), function () {
            var funcSource = test.split(' ==> ')[0];
            ...
            chai_1.expect(actualParamNames).deep.equal(expectedParamNames);
        });
    });
});
