...
describe('Constructing a Handler instance', () => {
    ...
    tests.forEach((test, i) => {
        it(`${test.pattern} WITH ${test.body}`, () => {
            ...
            let actualError = '';
            try {
                new handler_1.default(new pattern_1.default(test.pattern), test.body);
            }
            catch (ex) {
...
describe('Executing a handler against a pathname', () => {
    let tests = [
        ...
        {
            pattern: '/api/{...rest}',
            body: (rest, $yield) => `${$yield()}-${rest.slice(4, 7)}`,
            pathname: '/api/foo/bar/baz.html',
            downstream: rq => `${rq.pathname.slice(5, 8)}`,
            response: 'foo-bar'
        },
    ...
    tests.forEach((test, i) => {
        it(`${test.pattern} WITH ${test.body}`, () => {
            let handler = new handler_1.default(new pattern_1.default(test.pattern), test.body);
