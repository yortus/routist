...
variants.forEach(variant => describe(`Constructing a RuleSet instance (${variant.name})`, () => {
    ...
    let ruleSet = {
        ...
        '/bar': () => val('bar'),
        '/baz': () => val('baz'),
        '/*a*': async ($next) => val(`---${await ifUnhandled(await $next(), () => err('no downstream!'))}---`),
        ...
        'api/fo* #2': async ($req, $next) => val(`fo2-(${ifUnhandled(await $next($req), 'NONE')})`),
        'api/fo* #1': async ($req, $next) => val(`fo1-(${ifUnhandled(await $next($req), 'NONE')})`),
        'api/foo ': async ($req, $next) => val(`${ifUnhandled(await $next($req), 'NONE')}!`),
        'api/foo': () => val('FOO'),
        'api/foot': () => val('FOOt'),
        'api/fooo': () => val('fooo'),
        'api/bar': () => val(UNHANDLED),

        'zzz/{...rest}': async ($next, rest) => val(`${ifUnhandled(await $next({address: rest.split('').reverse().join('')}), 'NONE')}`),
    ...
    let multimethod = new Multimethod<{address:string}, string>({getDiscriminant: r => r.address}, ruleSet);
